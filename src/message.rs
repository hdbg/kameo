    ...
    R: Reply + ?Sized,
{
    ...
    /// Creates a new context with a different reply type, discarding the current reply sender.
    ///
    /// This is used internally by code generated by the `#[messages]` macro to dispatch
    /// enum variants to individual message handlers that accept a `ctx` parameter with a
    /// different reply type.
    #[doc(hidden)]
    pub fn __internal_fork<R2: Reply>(&self) -> Context<A, R2> {
        Context {
    ...
    /// Returns whether the actor has been signaled to stop.
    ///
    /// This is used internally by code generated by the `#[messages]` macro to propagate
    /// the stop flag from a forked context back to the outer context.
    #[doc(hidden)]
